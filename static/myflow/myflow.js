(function($) {
	...
	myflow.rect = function(o, r) {
		...
		_o.text.text += (myflow.util.nextId(o)[o.props.instance.value] - 1);


		_rect = _r.rect(_o.attr.x, _o.attr.y, _o.attr.width, _o.attr.height,
				_o.attr.r).hide().attr(_o.attr);

		_img = _r.image(myflow.config.basePath + _o.img.src.replace("*", o["dictype"]),
		...
		var clickHandler = function(e, src) {
			...
			if (src.getId() == _id) {
				...
				// window.instype = _o.props.instance.value;
			} else {
				hideBox();
			}
		};
			...
			switch (_o.showType) {
				...
				case 'image&text' :
					...
					_text.attr({
						...
						y : ry + (rh - myflow.config.lineHeight) 
								+ myflow.config.lineHeight
					}).show();
					_img.attr({
								x : rx + (rw - _o.img.width) / 2,
	...
	myflow.init = function(c, o) {
		...
		$(_r).bind('addrect', function(e, type, o) {
			...
			var rect = new myflow.rect($.extend(true, {},
							myflow.config.tools.states[types[0]], o, {'dictype': types[1]}), _r)
		...
		var addpathHandler = function(e, from, to) {
			...
			$.each(_paths, function (index, item) 
				...
				// if (item != null)
			 //            		if (item.from().text() && item.to().text() )
			 //                  			isadd = false;
		  //           		});
		...
		if (myflow.config.editable) {
			...
			$('#myflow_tools .node').hover(function() {
					...
					// myflow.config.tools.states[type].text.text = name;
					// myflow.config.tools.states[type].props.text.value = name + myflow.config.nextId;
					// myflow.config.tools.states[type].props.desc.value = $(this).attr('desc');
					// myflow.config.tools.states[type].props.instance.value = $(this).attr('instance');
					// myflow.config.tools.states[type].props.type.value = $(this).attr('type');
					if (type == 'imgls')
					{
						myflow.config.tools.states.imgls.text.text = name;
						myflow.config.tools.states.imgls.props.text.value = name+'_' + (myflow.config.nextId[name] || 1);
						myflow.config.tools.states.imgls.props.desc.value = $(this).attr('desc');
						myflow.config.tools.states.imgls.props.instance.value = $(this).attr('instance');
						myflow.config.tools.states.imgls.props.type.value = $(this).attr('type');
					}
					else if(type == 'imgvgtls')
					{
						myflow.config.tools.states.imgvgtls.text.text = name;
						myflow.config.tools.states.imgvgtls.props.text.value = name+'_'  + (myflow.config.nextId[name] || 1);
						myflow.config.tools.states.imgvgtls.props.desc.value = $(this).attr('desc');
						myflow.config.tools.states.imgvgtls.props.instance.value = $(this).attr('instance');
						myflow.config.tools.states.imgvgtls.props.type.value = $(this).attr('type');
					}
					else if(type == 'imgvshls')
					{
						myflow.config.tools.states.imgvshls.text.text = name;
						myflow.config.tools.states.imgvshls.props.text.value = name+'_'  + (myflow.config.nextId[name] || 1);
						myflow.config.tools.states.imgvshls.props.desc.value = $(this).attr('desc');
						myflow.config.tools.states.imgvshls.props.instance.value = $(this).attr('instance');
						myflow.config.tools.states.imgvshls.props.type.value = $(this).attr('type');
					}
					else if(type == 'pc')
					{
						myflow.config.tools.states.pc.text.text = name;
						myflow.config.tools.states.pc.props.text.value = name+'_'  + (myflow.config.nextId[name] || 1);
						myflow.config.tools.states.pc.props.desc.value = $(this).attr('desc');
						myflow.config.tools.states.pc.props.instance.value = $(this).attr('instance');
						myflow.config.tools.states.pc.props.type.value = $(this).attr('type');
					}
					else if(type == 'pswitch')
					{
						myflow.config.tools.states.pswitch.text.text = name;
						myflow.config.tools.states.pswitch.props.text.value = name+'_'  + (myflow.config.nextId[name] || 1);
						myflow.config.tools.states.pswitch.props.desc.value = $(this).attr('desc');
						myflow.config.tools.states.pswitch.props.instance.value = $(this).attr('instance');
						myflow.config.tools.states.pswitch.props.type.value = $(this).attr('type');
					}
					else if(type == 'pgate')
					{
						myflow.config.tools.states.pgate.text.text = name;
						myflow.config.tools.states.pgate.props.text.value = name+'_'  + (myflow.config.nextId[name] || 1);
						myflow.config.tools.states.pgate.props.desc.value = $(this).attr('desc');
						myflow.config.tools.states.pgate.props.instance.value = $(this).attr('instance');
						myflow.config.tools.states.pgate.props.type.value = $(this).attr('type');
					}
			...
			$('#myflow_clear').click(function() {// 清屏
				_states={};
				_paths={};
				var childs = document.getElementById("myflow").children[0].children;				
				$(childs).each(function(index,element){
					element.remove();
				});
				myflow.config.tools.clear.onclick("");
			});
