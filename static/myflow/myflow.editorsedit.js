...
$.extend(true, myflowedit.editors, {
	...
	textEditor : function(){
		...
		this.init = function(props, k, div, src, r){
			_props=props; _k=k; _div=div; _src=src; _r=r;
			$('<input  placeholder="请输入设备名称" style="width:100%;color:#000;" class="form-control topself_input"/>').val(props[_k].value).change(function(){var reg = new RegExp(/((2[0-4]\d|25[0-5]|[01]?\d\d?)\.){3}(2[0-4]\d|25[0-5]|[01]?\d\d?)/g);
				props[_k].value = $(this).val();
				$(_r).trigger('textchange', [$(this).val(), _src]);
			}).appendTo('#'+_div);
