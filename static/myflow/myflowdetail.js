(function($) {
		...
		path : {// 路径转换
			...
			props : {
				text : {
					...
					value : '',
					editor : function() {
						return new myflowdetail.editors.textEditor();
					}
				}
			}
		},
	...
	myflowdetail.rect = function(o, r) {

		var _this = this, _uid = myflowdetail.util.nextId(), _o = $.extend(true, {},
				myflowdetail.config.rect, o), _id = 'rect' + _uid, _r = r, // Raphael画笔
		_rect, _img, // 图标
		_name, // 状态名称
		_text, // 显示文本
		_ox, _oy; // 拖动时，保存起点位置;
		...
		_img = _r.image(myflowdetail.config.basePath + _o.img.src.replace("*", o["dictype"]),
		...
		_name = _r.text(
				...
				_o.attr.y + myflowdetail.config.lineHeight / 2, _o.name.text).hide()
				.attr(_o.name);
		_text = _r.text(
				_o.attr.x + _o.img.width + (_o.attr.width - _o.img.width) / 2,
				_o.attr.y + (_o.attr.height - myflowdetail.config.lineHeight) / 2
						+ myflowdetail.config.lineHeight, _o.text.text).hide()
				.attr(_o.text);// 文本
		...
		var dragMove = function(dx, dy) {// 拖动中
			if (!myflowdetail.config.editable)
				return;
		...
		var clickHandler = function(e, src) {
			...
			if (src.getId() == _id) {
				...
				// ----------------通过点击获取节点的信息-----------------------
				window.getType = _o.props.type.value;
				window.getVMName = _o.props.text.value;
				window.getVMIns = _o.vm;
			} else {
				hideBox();
			}
		};
			...
			switch (_o.showType) {
				...
				case 'image&text' :
					...
					_text.attr({
						...
						y : ry + (rh - myflowdetail.config.lineHeight) 
								+ myflowdetail.config.lineHeight
					}).show();
					_img.attr({
								x : rx + (rw - _o.img.width) / 2,
								y : ry + (rh - _o.img.height) / 2
	...
	myflowdetail.path = function(o, r, from, to) {
		//
		var _this = this, _r = r, _o = $.extend(true, {}, myflowdetail.config.path), _path, _arrow, _text, _textPos = _o.text.textPos, _ox, _oy, _from = from, _to = to, _id = 'path'
				+ myflowdetail.util.nextId(), _dotList, _autoText = true;
		//alert(_dotList);
		// 点
				...
				switch (_t) {
					case 'from' :
						if (_rt && _rt.right() && _rt.right().type() == 'to') {
							_rt.right().pos(myflowdetail.util.connPoint(
									_to.getBBox(), _pos));
						}
						if (_rt && _rt.right()) {
							_rt
									.pos(myflowdetail.util.center(_pos, _rt.right()
													.pos()));
						}
						break;
					case 'big' :
						...
						if (_lt && _lt.left() && _lt.left().type() == 'from') {
							_lt.left().pos(myflowdetail.util.connPoint(_from
											.getBBox(), _pos));
						}
						if (_rt && _rt.right()) {
							_rt
						...
						if (_lt && _lt.left()) {
							_lt.pos(myflowdetail.util.center(_pos, _lt.left().pos()));
						}
						// 三个大点在一条线上，移除中间的小点
						var pos = {
							x : _pos.x,
						...
						if (myflowdetail.util.isLine(_lt.left().pos(), pos, _rt
										.right().pos())) {
							_t = 'small';
					...
					case 'small' :// 移动小点时，转变为大点，增加俩个小点
						if (_lt && _rt && !myflowdetail.util.isLine(_lt.pos(), {
									x : _pos.x,
									y : _pos.y
								}, _rt.pos())) {
							...
							var lt = new dot('small', myflowdetail.util.center(_lt
													.pos(), _pos), _lt, _lt
											.right());
							_lt.right(lt);
							_lt = lt;

							var rt = new dot('small', myflowdetail.util.center(_rt
													.pos(), _pos), _rt.left(),
									_rt);
							_rt.left(rt);
							_rt = rt;
		...
		function dotList() {
			...
			_fromPos = myflowdetail.util.connPoint(_fromBB, {
						x : _toBB.x + _toBB.width / 2,
						y : _toBB.y + _toBB.height / 2
					});
			_toPos = myflowdetail.util.connPoint(_toBB, _fromPos);
			...
			this.toPathString = function() {
				...
				// 箭头路径
				var arrPos = myflowdetail.util.arrow(d.left().pos(), d.pos(),
		...
		var rectresizeHandler = function(e, src) {
			...
			if (_from && _from.getId() == src.getId()) {
				...
				var p = myflowdetail.util.connPoint(_from.getBBox(), rp);
			...
			if (_to && _to.getId() == src.getId()) {
				...
				var p = myflowdetail.util.connPoint(_to.getBBox(), rp);
	...
	myflowdetail.props = function(o, r) {
		// alert(myflow_propsdetail, this.propsdiv)
		var _this = this, _pdiv = $('#myflow_propsdetail').hide().draggable({
					handle : '#myflow_props_handledetail'
				}).resizable().css(myflowdetail.config.props.attr).bind('click',
				function() {
					return false;
	...
	// 初始化流程
	myflowdetail.init = function(c, o) {
		...
		$.extend(true, myflowdetail.config, o);
		...
		$(document).click(function() {
					...
					$(_r).trigger('showprops', [myflowdetail.config.props.props, {
										getId : function() {
											return '00000000';
										}
									}]);
		...
		$(_r).bind('addrect', function(e, type, o) {
			// $('body').append(type+', ');
			//********
			var rect = new myflowdetail.rect($.extend(true, {},
							myflowdetail.config.tools.states[type], o, {'dictype': types[1]}), _r)
		...
		var addpathHandler = function(e, from, to) {
			...
			$.each(_paths, function (index, item) 
	            	{ 
	            		if (item != null)
		            		if (from.text() == item.from().text() && to.text() == item.to().text() )
		                  			isadd = false;
	            	});

			if (isadd)
			{
				var path = new myflowdetail.path({}, _r, from, to);
		...
		if (myflowdetail.config.editable) {
			...
			$('#myflow_toolsdetail .node').hover(function() {
					//********
					var type = $(this).attr('type');
					var name = $(this).attr('name');
					myflowdetail.config.tools.states[type].text.text = name;
					myflowdetail.config.tools.states[type].props.text.value = name + myflowdetail.config.nextId;
					myflowdetail.config.tools.states[type].props.desc.value = $(this).attr('desc');
					myflowdetail.config.tools.states[type].props.instance.value = $(this).attr('instance');
					myflowdetail.config.tools.states[type].props.type.value = $(this).attr('type');
					// if (type == 'imgls')
					// {
					// 	myflowedit.config.tools.states.imgls.text.text = name;
					// 	myflowedit.config.tools.states.imgls.props.text.value = name + myflowedit.config.nextId;
					...
					// 	myflowedit.config.tools.states.imgvgtls.props.text.value = name + myflowedit.config.nextId;
					...
					// 	myflowedit.config.tools.states.imgvshls.props.text.value = name + myflowedit.config.nextId;
			...
			$('#myflow_toolsdetail .selectable').click(function() {
			...
			$('#myflow_toolsdetail .state').each(function() {
			...
			$('#myflow_cleardetail').click(function() {// 清屏
				_states={};
				_paths={};
				var childs = document.getElementById("myflowdetail").children[0].children;				
				$(childs).each(function(index,element){
					element.remove();
				});
				myflowdetail.config.tools.clear.onclick("");
			});

			$('#myflow_savedetail').click(function() {// 保存
						var data = '{states:{';
						debugger;
						var ary = [];
						...
						if (data.substring(data.length - 1, data.length) == ',')
							data = data.substring(0, data.length - 1);
						data += '},props:{props:{';
						for (var k in myflowdetail.config.props.props) {
							data += k + ":{value:'"
									+ myflowdetail.config.props.props[k].value
									+ "'},";
						}
						if (data.substring(data.length - 1, data.length) == ',')
							data = data.substring(0, data.length - 1);
						...
						myflowdetail.config.tools.save.onclick(data);
					});

			// 属性框
			new myflowdetail.props({}, _r);
		}
		// 恢复
		if (o.restore) {
			...
			if (data.states) {
				for (var k in data.states) {
					var rect = new myflowdetail.rect(
							$
									.extend(
											true,
											{},
											myflowdetail.config.tools.states[data.states[k].type],
											data.states[k], {"dictype": data.states[k]["dictype"]}), _r);
			...
			if (data.paths) {
				for (var k in data.paths) {
					var p = new myflowdetail.path($.extend(true, {},
									myflowdetail.config.tools.path, data.paths[k]),
		...
		// 历史状态
		var hr = myflowdetail.config.historyRects, ar = myflowdetail.config.activeRects;
	...
	// 添加jquery方法
	$.fn.myflowdetail = function(o) {
		return this.each(function()
		{
			//debugger;
			//myflow.config.tools.save = null;
			myflowdetail.init(this, o);
		});
	};

	$.myflowdetail = myflowdetail;
})(jQuery);