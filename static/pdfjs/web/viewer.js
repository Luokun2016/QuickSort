...
function configure(PDFJS) {
  PDFJS.imageResourcesPath = '/statics/pdfjs/web/images/';
  PDFJS.workerSrc = '/statics/pdfjs/build/pdf.worker.js';
  PDFJS.cMapUrl = '/statics/pdfjs/web/cmaps/';
...
function webViewerInitialized() {
  ...
  // --------------------修改原有的全屏操作，实现包含pdf的iframe全屏-----------------------
  // document.getElementById('presentationMode').addEventListener('click',
  //   SecondaryToolbar.presentationModeClick.bind(SecondaryToolbar));
  
  function getFrame(){
    var frames = window.parent.document.getElementsByTagName("iframe"); //获取父页面所有iframe
    for(var i=0;i<frames.length;i++){ //遍历，匹配时弹出id
      if(frames[i].contentWindow == window){
        return frames[i];  // 返回包含当前pdf的iframe
      }
    }
  }
  ...
  function iframeFull(){
    ...
    // 全屏时隐藏全屏按钮
    document.getElementById('presentationMode').style.visibility = 'hidden';
  }

  // 退出全屏时，显示全屏按钮
  window.parent.document.onwebkitfullscreenchange = function () {
      if (!window.parent.document.webkitIsFullScreen) {
          document.getElementById('presentationMode').style.visibility = 'visible';
      }
  };
  window.parent.document.onmozfullscreenchange = function () {
      if (!window.parent.document.mozFullScreen) {
          document.getElementById('presentationMode').style.visibility = 'visible';
      }
  };

  document.getElementById('presentationMode').addEventListener('click', iframeFull);
  // -----------------以上是修改后的全屏功能-------------------------------------------
