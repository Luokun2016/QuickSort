# -*- coding: utf-8 -*-
'''
使用libvirt操作虚拟机的库文件，不要对其进行任何修改
'''

# Utility functions used for guest installation
import libvirt
import virtinst.util as util
from sysmgr.IPy import IP
import re
import time
import libxml2
...
class ConnServer(object):
    ...
    def add_vm(self, name, ram, cpu, image,  osindex,addtype):
        """
        Create VM function
        """
        ram = int(ram) * 1024
        ...
        xml = """<domain type='%s'>
                  ...
                  <vcpu placement='static'>%s</vcpu>""" % (dom_type, name, ram, ram, cpu)
        imgtype = image.find('qcow2')
        if imgtype > -1:
            imgtype = 'qcow2'
        else:
            imgtype = 'raw'
        ...
        image1 = None
        if image.find('img') > -1:
            cmd = "cp " + image + " " + IMAGE_PATH + name + ".img"
            image1 = IMAGE_PATH + name + ".img"
        else:
            cmd = "cp " + image + " " + IMAGE_PATH + name + ".qcow2"
            image1 = IMAGE_PATH + name + ".qcow2"
        ...
        xml += """<os>
                  ...
                  <devices>
                    <emulator>%s</emulator>""" % (osindex, machine, emulator)

        xml += """<disk type='file' device='disk'>
                      <driver name='qemu' type='%s'/>
                      <source file='%s'/>""" % (imgtype,  image1)
        ...
        if addtype=='vm':
            ...
            xml += """
                    <interface type='direct'>
                        ...
                        <interface type='direct'>
                        <source dev='2' mode='bridge'/>
                        </interface>
                        <interface type='direct'>
                        <source dev='3' mode='bridge'/>
                        </interface>
                        <interface type='direct'>
                        <source dev='4' mode='bridge'/>
                    </interface>"""
            xml += """
                <interface type='network'>
                    <source network='default'/>"""
    ...
    def memory_get_usage(self):
        ...
        percent=mem['MemUsed']/mem['MemTotal']*100
        percent=round(percent,1)
        return mem['MemTotal'], mem['MemUsed'], percent
    ...
    def disk_get_usage(self):
        ...
        free = (disk.f_bavail * disk.f_bsize)/1048576  
        alldisk = (disk.f_blocks * disk.f_frsize)/1048576  
        diskusage = (disk.f_blocks - disk.f_bfree) * disk.f_frsize /1048576 
        # print(str(alldisk*1048576 )+"  "+str(diskusage*1048576 ))
        try:  
            percent = (float(diskusage) * 100 ) / float(alldisk)
        except ZeroDivisionError:  
            percent = 0
        percent=round(percent,1)      
        return alldisk, diskusage, free, percent
    ...
    def cpu_get_usage(self):
        ...
        if not cpustr:
            return 0
        usni1=long(cpustr[1])+long(cpustr[2])+long(cpustr[3])+long(cpustr[4])+long(cpustr[5])+long(cpustr[6])+long(cpustr[7]) 
        usn1=long(cpustr[1])+long(cpustr[2])+long(cpustr[3])
        time.sleep(2)
        cpustr=self._read_cpu_usage()
        if not cpustr:
            return 0
        usni2=long(cpustr[1])+long(cpustr[2])+long(cpustr[3])+long(cpustr[4])+long(cpustr[5])+long(cpustr[6])+long(cpustr[7]) 
    ...
    def vds_cpu_usage(self, vname):
        ...
        if dom.info()[0] == 1:
            ...
            cpu_usage = 100 * float(diff_usage) / float(1 * nbcore * 10 ** 9L)
    ...
    def vds_get_info(self, vname):
        ...
        def get_networks(ctx):
            ...
            for interface in ctx.xpathEval('/domain/devices/interface'):
                ...
                nic = interface.xpathEval('source/@dev|source/@mode')[0].content
